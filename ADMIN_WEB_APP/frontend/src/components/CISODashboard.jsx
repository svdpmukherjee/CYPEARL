...
const matchEmployeeToPersonas = (employee, personas, featureList) => {
  ...
  const matches = personas
    .map((persona) => {
      const personaVector = features.map(
        (f) => persona.trait_zscores?.[f] || 0,
      );
      return {
        persona_id: persona.persona_id,
        similarity: cosineSimilarity(employeeVector, personaVector),
      };
    })
    .sort((a, b) => b.similarity - a.similarity);
  return {
    best_match: matches[0] || { persona_id: null, similarity: 0 },
    all_matches: matches,
  };
};
...
// Helper to extract AI-generated name from hybrid name format
// Converts "LO-ANL-CUR-RPT: Cautious Security Minded" to "Cautious Security Minded"
const getAiPersonaName = (persona) => {
  if (!persona) return "Unknown";
  // If llm_name is directly available, use it
  if (persona.llm_name) return persona.llm_name;
  // Otherwise extract from hybrid name format
  if (persona.name && persona.name.includes(": ")) {
    return persona.name.split(": ").slice(1).join(": ");
  }
  return persona.name || `Persona ${persona.persona_id + 1}`;
};
  ...
  const handleConfigUpload = async (file) => {
    ...
    try {
      ...
      if (!config.personas || !config.prompt_templates) {
        throw new Error(
          "Invalid config. Must include personas and prompt_templates.",
        );
      }
    ...
    <div className="space-y-6">
      <div className="text-center mb-8">
        ...
        <p className="text-gray-500">
          Upload the Admin-published configuration and your employee assessment
          data
        </p>
      </div>
      ...
      <div className="grid md:grid-cols-2 gap-6">
        ...
        <Card className="p-6">
          ...
          {adminConfig ? (
            <div className="p-4 bg-green-50 border border-green-200 rounded-lg">
              ...
              <div className="grid grid-cols-2 gap-2 text-sm text-green-700">
                ...
                <div>
                  Prompts:{" "}
                  {Object.keys(adminConfig.prompt_templates || {}).length}
                </div>
            ...
            <div
              className="border-2 border-dashed border-gray-300 rounded-lg p-8 text-center hover:border-purple-400 cursor-pointer"
              onClick={() => configFileRef.current?.click()}
            >
              <Upload className="mx-auto text-gray-400 mb-2" size={32} />
              <p className="text-sm text-gray-600">
                Upload admin_published_config_v2.json
              </p>
            </div>
          )}
          {errors.config && (
          ...
          <input
            ...
            type="file"
            accept=".json"
            onChange={(e) => handleConfigUpload(e.target.files[0])}
            className="hidden"
          />
        </Card>
        ...
        <Card className="p-6">
          ...
          <input
            ...
            type="file"
            accept=".csv,.json"
            onChange={(e) => handleEmployeeUpload(e.target.files[0])}
            className="hidden"
          />
        </Card>
      </div>
      ...
      <div className="flex justify-end">
        <Button
          onClick={onNext}
          disabled={!adminConfig || employeeData.length === 0}
        >
...
const MatchStep = ({
  ...
  const matchResults = useMemo(() => {
    ...
    const features =
      adminConfig.matching_features ||
      (adminConfig.personas[0]?.trait_zscores
        ? Object.keys(adminConfig.personas[0].trait_zscores)
        : []);
    ...
    Object.values(distribution).forEach((d) => {
      if (d.count > 0)
        d.avg_similarity =
          d.employees.reduce((sum, e) => sum + e.similarity, 0) / d.count;
    });

    return {
      employee_matches: employeeMatches,
      distribution: Object.values(distribution).sort(
        (a, b) => b.count - a.count,
      ),
  ...
  useEffect(() => {
    if (matchResults) onSetMatches(matchResults);
  }, [matchResults, onSetMatches]);
  ...
  return (
    <div className="space-y-6">
      <div className="text-center mb-8">
        ...
        <p className="text-gray-500">
          Matched using cosine similarity on{" "}
          {
            (
              adminConfig.matching_features ||
              (adminConfig.personas[0]?.trait_zscores
      ...
      <div className="grid grid-cols-4 gap-4">
        <Card className="p-4">
          <div className="text-2xl font-bold">
            {matchResults.total_employees}
          </div>
          <div className="text-sm text-gray-500">Employees</div>
        </Card>
        <Card className="p-4">
          <div className="text-2xl font-bold">
            {matchResults.distribution.filter((d) => d.count > 0).length}
          </div>
          <div className="text-sm text-gray-500">Personas Matched</div>
        </Card>
        <Card className="p-4">
          <div className="text-2xl font-bold text-red-600">
            {matchResults.distribution
              .filter((d) =>
                ["CRITICAL", "HIGH"].includes(d.persona.risk_level),
              )
        ...
        <Card className="p-4">
          <div className="text-2xl font-bold">
            {(
              (matchResults.distribution.reduce(
                (s, d) => s + d.avg_similarity * d.count,
                0,
              ) /
                matchResults.total_employees) *
              100
      ...
      <Card className="p-6">
        ...
        <div className="space-y-2">
          {matchResults.distribution
            ...
            .map((d) => (
              <div
                key={d.persona.persona_id}
                className="flex items-center gap-4 p-3 bg-gray-50 rounded-lg"
              >
                <div className="flex-1">
                  <div className="flex items-center gap-2">
                    <span className="font-medium">
                      {getAiPersonaName(d.persona)}
                    </span>
                    <Badge color={riskColors[d.persona.risk_level]}>
                      {d.persona.risk_level}
                    </Badge>
                  </div>
                ...
                <div className="text-right">
                  <div className="font-bold">
                    ...
                    <span className="text-gray-400 font-normal">
                      (
                      {((d.count / matchResults.total_employees) * 100).toFixed(
                        0,
                      )}
                      %)
                    </span>
                  ...
                  <div className="text-xs text-gray-500">
                    Similarity: {(d.avg_similarity * 100).toFixed(0)}%
                  </div>
                </div>
              </div>
            ))}
      ...
      <div className="flex justify-between">
        ...
        <Button onClick={onNext}>
          Configure LLM & Prompts <ChevronRight size={16} />
        </Button>
      </div>
    </div>
  );
...
const ConfigureStep = ({
  ...
  const personasWithMatches =
    matchResults?.distribution?.filter((d) => d.count > 0) || [];
  const riskColors = {
    CRITICAL: "red",
    HIGH: "orange",
    MEDIUM: "yellow",
  ...
  const handleConfigSelect = (personaId, llmId, promptId) => {
    onSelectConfig({
      ...selectedConfigs,
      [personaId]: { llm_id: llmId, prompt_config: promptId },
    });
  };
  ...
  const estimatedCost = useMemo(() => {
    ...
    selectedPersonas.forEach((pid) => {
      const personaMatch = personasWithMatches.find(
        (d) => d.persona.persona_id === pid,
      );
      if (!personaMatch) return;
      const config = selectedConfigs[pid] || {
        llm_id: personaMatch.persona.validated_configurations?.[0]?.llm_id,
        prompt_config:
          personaMatch.persona.validated_configurations?.[0]?.prompt_config ||
          "stats",
      };
      // Get cost from fidelity_matrix first, fallback to validated_configurations
      const matrixEntry =
        personaMatch.persona.fidelity_matrix?.[config?.llm_id]?.[
          config?.prompt_config
        ];
      const costPerCall =
        matrixEntry?.cost ||
        personaMatch.persona.validated_configurations?.find(
          (c) =>
            c.llm_id === config?.llm_id &&
            c.prompt_config === config?.prompt_config,
        )?.cost_per_call ||
        0.005; // default ~$0.005 per call
  ...
  return (
    <div className="space-y-6">
      <div className="text-center mb-8">
        <h2 className="text-2xl font-bold text-gray-900 mb-2">
          Select LLM & Prompt Configuration
        </h2>
        <p className="text-gray-500">
          Choose the LLM and prompt combination for each persona. View fidelity
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <Button
              ...
              onClick={() =>
                onSelectPersona(
                  personasWithMatches.map((d) => d.persona.persona_id),
                )
              }
            >
              Select All
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex border rounded-lg overflow-hidden">
              <button
                ...
                className={`px-3 py-1 text-sm ${viewMode === "matrix" ? "bg-indigo-100 text-indigo-700" : "bg-white"}`}
              >
                <Grid size={14} />
              </button>
              <button
                onClick={() => setViewMode("list")}
                className={`px-3 py-1 text-sm ${viewMode === "list" ? "bg-indigo-100 text-indigo-700" : "bg-white"}`}
              >
                <List size={14} />
              </button>
            </div>
      ...
      <div className="space-y-4">
        {personasWithMatches.map((d) => {
          ...
          const currentConfig = selectedConfigs[persona.persona_id] || {
            llm_id:
              persona.validated_configurations?.[0]?.llm_id ||
              adminConfig.llm_options?.[0]?.id,
            prompt_config:
              persona.validated_configurations?.[0]?.prompt_config || "stats",
          };

          // Get fidelity and cost from fidelity_matrix (primary source)
          const matrixEntry =
            persona.fidelity_matrix?.[currentConfig.llm_id]?.[
              currentConfig.prompt_config
            ];
          const currentFidelity =
            matrixEntry?.fidelity ||
            persona.validated_configurations?.find(
              (c) =>
                c.llm_id === currentConfig.llm_id &&
                c.prompt_config === currentConfig.prompt_config,
            )?.fidelity_score ||
            0.85;
          const currentCost =
            matrixEntry?.cost ||
          ...
          const currentLlmName =
            adminConfig.llm_options?.find((l) => l.id === currentConfig.llm_id)
              ?.name || currentConfig.llm_id;
          const currentPromptName =
            adminConfig.prompt_templates?.[currentConfig.prompt_config]?.name ||
            currentConfig.prompt_config;
              ...
              <div className={`p-4 ${isSelected ? "bg-indigo-50" : ""}`}>
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2">
                      <span className="font-semibold">
                        {getAiPersonaName(persona)}
                      </span>
                      <Badge color={riskColors[persona.risk_level]}>
                        {persona.risk_level}
                      </Badge>
                      <span className="text-sm text-gray-500">
                        • {d.count} employees
                  ...
                  <div className="text-right mr-4">
                    ...
                    <div className="text-xs text-gray-500">
                      {currentPromptName} prompt
                    </div>
                    <div className="flex items-center gap-2 justify-end">
                      <Badge color="green">
                        {(currentFidelity * 100).toFixed(0)}% fidelity
                      </Badge>
                      <span className="text-xs text-gray-400">
                        ${currentCost.toFixed(4)}/call
                      </span>
                    </div>
                  </div>
                  <button
                    onClick={() =>
                      setExpandedPersona(isExpanded ? null : persona.persona_id)
                    }
                ...
                <div className="p-4 bg-gray-50 border-t">
                    ...
                    <table className="w-full text-sm">
                      <thead>
                        <tr className="text-left">
                          ...
                          {Object.values(adminConfig.prompt_templates).map(
                            (pt) => (
                      ...
                      <tbody>
                        {adminConfig.llm_options.map((llm) => (
                          <tr key={llm.id} className="border-t">
                            ...
                            {Object.keys(adminConfig.prompt_templates).map(
                              (promptId) => {
                                const matrixEntry =
                                  persona.fidelity_matrix?.[llm.id]?.[promptId];
                                const fidelity = matrixEntry?.fidelity || 0;
                                const cost = matrixEntry?.cost || 0;
                                const isCurrentSelection =
                                  currentConfig.llm_id === llm.id &&
                                  currentConfig.prompt_config === promptId;
                                ...
                                return (
                                  <td key={promptId} className="py-2 px-2">
                                    ...
                                    >
                                      ...
                                      <div
                                        className={`text-xs ${isCurrentSelection ? "text-indigo-200" : "text-gray-400"}`}
                                      >
                                        ${cost.toFixed(3)}/call
                                      </div>
                                    </button>
                  ...
                  <div className="mt-4 flex items-center gap-4 text-xs text-gray-500">
                    ...
                    <span className="flex items-center gap-1">
                      <span className="w-3 h-3 bg-green-500 rounded" /> ≥95%
                    </span>
                    <span className="flex items-center gap-1">
                      <span className="w-3 h-3 bg-green-400 rounded" /> 90-94%
                    </span>
                    <span className="flex items-center gap-1">
                      <span className="w-3 h-3 bg-yellow-500 rounded" /> 85-89%
                    </span>
                    <span className="flex items-center gap-1">
                      <span className="w-3 h-3 bg-red-500 rounded" /> &lt;85%
                    </span>
                  </div>
                  ...
                  <div className="mt-4 p-3 bg-white rounded-lg border">
                    <h5 className="text-sm font-medium text-gray-700 mb-2">
                      Behavioral Targets for this Persona:
                    </h5>
                    <div className="grid grid-cols-3 gap-4 text-sm">
                      <div>
                        ...
                        <span className="ml-2 font-medium text-red-600">
                          {(
                            ((
                              persona.behavioral_targets ||
                              persona.behavioral_statistics
                            )?.phishing_click_rate || 0) * 100
                          ).toFixed(1)}
                          %
                        </span>
                      </div>
...
const PromptsStep = ({
  ...
  const selectedPersonaDetails =
    matchResults?.distribution?.filter((d) =>
      selectedPersonas.includes(d.persona.persona_id),
    ) || [];
  ...
  const getSamplePrompt = (promptType, persona) => {
    ...
    const variables = {
      ...
      click_rate: (
        persona.behavioral_targets.phishing_click_rate * 100
      ).toFixed(1),
      ...
      response_time: Math.round(
        persona.behavioral_targets.mean_response_latency_ms,
      ),
      hover_rate: (persona.behavioral_targets.hover_rate * 100).toFixed(1),
      inspection_rate: (
        persona.behavioral_targets.sender_inspection_rate * 100
      ).toFixed(1),
      ...
      // Reasoning examples
      reasoning_example_1:
        persona.prompt_variables?.reasoning_example_1 ||
        "Example not available",
      reasoning_example_2:
        persona.prompt_variables?.reasoning_example_2 ||
        "Example not available",
      reasoning_example_3:
        persona.prompt_variables?.reasoning_example_3 ||
        "Example not available",
      trait_details: Object.entries(persona.trait_zscores || {})
  ...
  return (
    <div className="space-y-6">
      <div className="text-center mb-8">
        ...
        <p className="text-gray-500">
          View the prompts that will be sent to LLMs. Optionally customize for
          your context.
        </p>
      </div>
      ...
      <Card className="p-2">
        <div className="flex gap-2">
          {Object.values(promptTemplates).map((pt) => {
            ...
            const usedByCount = selectedPersonas.filter(
              (pid) =>
      ...
      <div className="grid md:grid-cols-2 gap-6">
        ...
        <Card className="p-4">
          <div className="flex items-center justify-between mb-3">
            ...
            <div className="flex items-center gap-2">
              <button
                onClick={() =>
                  handleCopy(
                    promptTemplates[selectedPromptType]?.system_template,
                    "system",
                  )
                }
          ...
          {editingPrompt === "system" ? (
            <textarea
              value={
                customPrompts[selectedPromptType]?.system_template ||
                promptTemplates[selectedPromptType]?.system_template
              }
              onChange={(e) =>
                handleCustomize(
        ...
        <Card className="p-4">
          <div className="flex items-center justify-between mb-3">
            ...
            <div className="flex items-center gap-2">
              <button
                onClick={() =>
                  handleCopy(
                    promptTemplates[selectedPromptType]?.user_template,
                    "user",
                  )
                }
                className="p-1.5 hover:bg-gray-100 rounded"
          ...
          {editingPrompt === "user" ? (
            <textarea
              value={
                customPrompts[selectedPromptType]?.user_template ||
                promptTemplates[selectedPromptType]?.user_template
              }
              onChange={(e) =>
                handleCustomize(
      ...
      {selectedPersonaDetails.length > 0 && (
        <Card className="p-4">
          <h3 className="font-semibold mb-3 flex items-center gap-2">
            <Eye size={16} />
            Preview: Rendered Prompt for "
            {getAiPersonaName(selectedPersonaDetails[0].persona)}"
          </h3>
          <div className="p-3 bg-gray-50 rounded-lg text-xs max-h-64 overflow-auto">
            ...
            <pre className="whitespace-pre-wrap text-gray-700">
              {getSamplePrompt(
                selectedPromptType,
                selectedPersonaDetails[0].persona,
              ).system.substring(0, 1500)}
              ...
            </pre>
          </div>
        </Card>
      ...
      <Card className="p-4 bg-blue-50 border-blue-200">
        ...
        <div className="grid grid-cols-4 gap-2 text-xs text-blue-800">
          {[
            ...
            <code key={v} className="bg-blue-100 px-1 rounded">
              {v}
            </code>
          ))}
        </div>
      ...
      <div className="flex justify-between">
        ...
        <Button onClick={onNext}>
          Create Test Emails <ChevronRight size={16} />
        </Button>
      </div>
    </div>
  );
  ...
  const templates = [
    {
      ...
      sender: "it-security@company-alerts.com",
    ...
    },
    {
      ...
      sender: "delivery@postal-notify.com",
  ...
  return (
    <div className="space-y-6">
      <div className="text-center mb-8">
        ...
        <p className="text-gray-500">
          These emails will be sent to the LLM personas for behavioral
          prediction
        </p>
      </div>
        ...
        <div className="grid grid-cols-4 gap-3">
          {templates.map((t, i) => (
            <button
              key={i}
              onClick={() =>
                onAddEmail({ id: `email_${Date.now()}_${i}`, ...t })
              }
              className="p-3 border rounded-lg text-left hover:bg-gray-50 text-sm"
            >
              <div className="font-medium">{t.name}</div>
              <div className="flex gap-1 mt-1">
                <Badge color={t.urgency === "high" ? "red" : "gray"} size="sm">
                  {t.urgency}
                </Badge>
              </div>
            </button>
      ...
      <Card className="p-6">
        ...
        <div className="grid md:grid-cols-2 gap-4">
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setNewEmail((p) => ({ ...p, subject: e.target.value }))
              }
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setNewEmail((p) => ({ ...p, sender: e.target.value }))
              }
              className="w-full px-3 py-2 border rounded-lg"
        ...
        <div className="mt-4">
          ...
          <textarea
            value={newEmail.body}
            onChange={(e) =>
              setNewEmail((p) => ({ ...p, body: e.target.value }))
            }
            className="w-full px-3 py-2 border rounded-lg"
            rows={4}
        ...
        <div className="grid grid-cols-3 gap-4 mt-4">
          <select
            value={newEmail.urgency}
            onChange={(e) =>
              setNewEmail((p) => ({ ...p, urgency: e.target.value }))
            }
          ...
          <select
            value={newEmail.sender_type}
            onChange={(e) =>
              setNewEmail((p) => ({ ...p, sender_type: e.target.value }))
            }
            className="px-3 py-2 border rounded-lg"
          >
            <option value="unknown">Unknown Sender</option>
            <option value="familiar">Familiar Sender</option>
            <option value="authority">Authority Figure</option>
          </select>
          <select
            value={newEmail.framing}
            onChange={(e) =>
              setNewEmail((p) => ({ ...p, framing: e.target.value }))
            }
            className="px-3 py-2 border rounded-lg"
          >
          ...
          <div className="space-y-2">
            {testEmails.map((e) => (
              ...
              >
                ...
                <div className="flex items-center gap-2">
                  <Badge color={e.urgency === "high" ? "red" : "gray"}>
                    {e.urgency}
                  </Badge>
      ...
      <div className="flex justify-between">
        ...
        <Button onClick={onNext} disabled={testEmails.length === 0}>
          Run LLM Predictions <ChevronRight size={16} />
        </Button>
      </div>
    </div>
  );
...
const PredictStep = ({
  ...
  const costEstimate = useMemo(() => {
    ...
    selectedPersonaDetails.forEach((d) => {
      ...
      const configDetails =
        d.persona.validated_configurations.find(
          (c) =>
            c.llm_id === config?.llm_id &&
            c.prompt_config === config?.prompt_config,
    ...
    for (const personaMatch of selectedPersonaDetails) {
      ...
      for (const email of testEmails) {
        setProgress({
          current: completed,
          total: totalOps,
          persona: getAiPersonaName(persona),
          email: email.subject,
        });
        ...
        if (email.urgency === "high" && modifiers.urgency_high)
          clickRate *= modifiers.urgency_high.click_multiplier;
        if (email.sender_type === "authority" && modifiers.authority_sender)
          clickRate *= modifiers.authority_sender.click_multiplier;
        if (email.framing === "threat" && modifiers.threat_framing)
          clickRate *= modifiers.threat_framing.click_multiplier;

        clickRate = Math.min(0.95, Math.max(0.02, clickRate));
        const reportRate = Math.min(
          0.4,
          persona.behavioral_targets.report_rate * (1 - clickRate),
        );
  ...
  return (
    <div className="space-y-6">
      <div className="text-center mb-8">
        ...
        <p className="text-gray-500">
          Generate predictions using your selected LLM configurations
        </p>
      </div>
      ...
      <Card className="p-4">
        <div className="flex items-center gap-4">
          ...
          <div className="flex border rounded-lg overflow-hidden">
            <button
              onClick={() => setMode("llm")}
              className={`px-4 py-2 text-sm flex items-center gap-2 ${mode === "llm" ? "bg-indigo-100 text-indigo-700" : "bg-white"}`}
            >
              <Cpu size={14} /> LLM Simulation
            </button>
            <button
              onClick={() => setMode("statistical")}
              className={`px-4 py-2 text-sm flex items-center gap-2 ${mode === "statistical" ? "bg-indigo-100 text-indigo-700" : "bg-white"}`}
            >
      ...
      <Card className="p-6 bg-amber-50 border-amber-200">
        ...
        <p className="text-xs text-amber-700 mt-3">
          Formula: {selectedPersonas.length} personas × {testEmails.length}{" "}
          emails × {costEstimate.trialsPerEmail} trials ={" "}
            ...
            return (
              ...
              >
                ...
                <div className="flex items-center gap-3">
                  ...
                  <Badge color="green">
                    {(configDetails?.fidelity_score * 100).toFixed(0)}%
                  </Badge>
                </div>
              </div>
            );
          ...
          {running ? (
            <div className="space-y-4">
              ...
              <h3 className="text-lg font-semibold">
                Running {mode === "llm" ? "LLM" : "Statistical"} Predictions...
              </h3>
            ...
            <div className="space-y-4">
              ...
              <p className="text-gray-500">
                This will{" "}
                {mode === "llm"
                  ? "make API calls to selected LLMs"
                  : "calculate predictions statistically"}
              </p>
              <Button size="lg" onClick={handleRun}>
                <Play size={20} /> Run Predictions{" "}
                {mode === "llm"
                  ? `($${costEstimate.totalCost.toFixed(2)})`
                  : "(Free)"}
              </Button>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-green-700">
                {predictions.length} personas × {testEmails.length} emails
                analyzed
              </p>
            </div>
          </div>
      ...
      <div className="flex justify-between">
        ...
        <Button onClick={onNext} disabled={!predictions}>
...
const ReportStep = ({
  ...
  const orgStats = useMemo(() => {
    ...
    predictions.forEach((p) => {
      const avgClick =
        p.email_results.reduce((s, r) => s + r.click_rate, 0) /
        p.email_results.length;
      const avgReport =
        p.email_results.reduce((s, r) => s + r.report_rate, 0) /
        p.email_results.length;
    ...
    return {
      avgClickRate: weightedClick,
      avgReportRate: weightedReport,
      estClicks: Math.round(weightedClick * totalEmployees),
    };
    ...
    <div className="space-y-6">
        ...
        <div className="grid grid-cols-4 gap-6">
          <div>
            <div className="text-3xl font-bold">
              {(orgStats.avgClickRate * 100).toFixed(0)}%
            </div>
            <div className="text-indigo-200">Avg Click Rate</div>
          </div>
          <div>
            <div className="text-3xl font-bold">{orgStats.estClicks}</div>
            <div className="text-indigo-200">Est. Employees at Risk</div>
          </div>
          <div>
            <div className="text-3xl font-bold">
              {(orgStats.avgReportRate * 100).toFixed(0)}%
            </div>
            <div className="text-indigo-200">Would Report</div>
          </div>
          <div>
            <div className="text-3xl font-bold">{predictions.length}</div>
            <div className="text-indigo-200">Personas Analyzed</div>
          </div>
        </div>
      </Card>
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {predictions
              .sort((a, b) => {
                const aClick =
                  a.email_results.reduce((s, r) => s + r.click_rate, 0) /
                  a.email_results.length;
                const bClick =
                  b.email_results.reduce((s, r) => s + r.click_rate, 0) /
                  b.email_results.length;
                  ...
                  <tr key={p.persona.persona_id} className="border-b">
                    <td className="py-3">
                      <div className="font-medium">
                        {getAiPersonaName(p.persona)}
                      </div>
                      <div className="text-gray-500">{p.persona.archetype}</div>
                    </td>
                    ...
                    <td className="py-3">
                      <div className="text-xs">
                        ...
                        <span className="text-gray-500">
                          {p.config.prompt_name}
                        </span>
                      </div>
                    </td>
                    <td className="py-3">
                      <Badge color="green">
                        {(p.config.fidelity_score * 100).toFixed(0)}%
                      </Badge>
                    </td>
                    <td className="py-3">
                      <span
                        className={`font-bold ${avgClick > 0.4 ? "text-red-600" : avgClick > 0.25 ? "text-amber-600" : "text-green-600"}`}
                      >
                        {(avgClick * 100).toFixed(1)}%
                      </span>
                    </td>
        ...
        {predictions
          ...
          .map((p, i) => (
            <div key={p.persona.persona_id} className="mb-2">
              <strong>
                {i + 1}. {getAiPersonaName(p.persona)}
              </strong>{" "}
              ({p.employee_count} employees): Focus on{" "}
              {p.persona.distinguishing_high_traits?.slice(0, 2).join(", ")}{" "}
              awareness training.
            </div>
          ))}{" "}
      </Card>
      ...
      <div className="flex justify-between">
        ...
        <div className="flex gap-3">
          ...
          <Button variant="success" onClick={onStartOver}>
...
const CISODashboard = ({ onBack }) => {
  ...
  useEffect(() => {
    if (matchResults && selectedPersonas.length > 0) {
      ...
      selectedPersonas.forEach((pid) => {
        if (!newConfigs[pid]) {
          const persona = matchResults.distribution.find(
            (d) => d.persona.persona_id === pid,
          )?.persona;
          if (persona?.validated_configurations?.[0]) {
            ...
            newConfigs[pid] = {
              llm_id: rec.llm_id,
              prompt_config: rec.prompt_config,
            };
          }
        }
      });
          ...
          {WORKFLOW_STEPS.map((step, idx) => (
            <React.Fragment key={step.id}>
              ...
              >
                ...
                <span
                  className={`text-xs font-medium ${idx === currentStep ? "text-indigo-600" : idx < currentStep ? "text-green-600" : "text-gray-500"}`}
                >
                  {step.label}
                </span>
              </button>
              {idx < WORKFLOW_STEPS.length - 1 && (
                <div
                  className={`flex-1 h-0.5 mx-2 ${idx < currentStep ? "bg-green-500" : "bg-gray-200"}`}
                />
              )}
            </React.Fragment>
          ))}
      ...
      <div className="max-w-6xl mx-auto px-6 py-8">
        {currentStep === 0 && (
          <UploadStep
            ...
            onUploadEmployees={setEmployeeData}
            onUploadConfig={setAdminConfig}
            onNext={() => goToStep(1)}
          />
        )}
        {currentStep === 1 && (
        ...
        {currentStep === 2 && (
          <ConfigureStep
            ...
            onSelectPersona={setSelectedPersonas}
            onSelectConfig={setSelectedConfigs}
            onBack={() => goToStep(1)}
            onNext={() => goToStep(3)}
          />
        )}
        {currentStep === 3 && (
          <PromptsStep
            ...
            onSetCustomPrompts={setCustomPrompts}
            onBack={() => goToStep(2)}
            onNext={() => goToStep(4)}
          />
        )}
        {currentStep === 4 && (
          <EmailsStep
            ...
            onAddEmail={(e) => setTestEmails((p) => [...p, e])}
            onRemoveEmail={(id) =>
              setTestEmails((p) => p.filter((e) => e.id !== id))
            }
            onBack={() => goToStep(3)}
            onNext={() => goToStep(5)}
          />
        ...
        {currentStep === 5 && (
          <PredictStep
            ...
            onRunPredictions={setPredictions}
            onBack={() => goToStep(4)}
            onNext={() => goToStep(6)}
          />
        )}
