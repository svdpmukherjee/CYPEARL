...
export const ValidationTab = ({
  ...
  // Helper to get persona display name - AI name only
  const getPersonaName = (cluster) => {
    ...
    return aiName || label?.name || `Persona ${cluster.cluster_id + 1}`;
  };

  const eta2ByOutcome = clusteringResult.metrics?.eta_squared_by_outcome || {};
  const eta2Data = Object.entries(eta2ByOutcome)
    .map(([outcome, value]) => ({
  ...
  const behaviorComparisonData = clusters
    .map((c) => ({
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-gray-600">
              Clusters explain{" "}
              <strong>{(effectSizeMean * 100).toFixed(1)}%</strong> of variance
              in phishing behavior on average
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* η² by Outcome */}
        <ChartCard
          title="Prediction Strength by Outcome"
          subtitle="How well clusters predict each behavioral metric (η²)"
        >
          <ResponsiveContainer width="100%" height={300}>
            <BarChart data={eta2Data} layout="vertical">
              ...
              <ReferenceLine
                ...
                stroke="#f59e0b"
                strokeDasharray="5 5"
                label={{ value: "Medium", fontSize: 10, position: "top" }}
              />
              <ReferenceLine
                x={0.14}
                stroke="#22c55e"
                strokeDasharray="5 5"
                label={{ value: "Large", fontSize: 10, position: "top" }}
              />
        ...
        <ChartCard
          title="Phishing Click Rate by Persona"
          subtitle="Higher = more vulnerable to phishing"
        >
          <ResponsiveContainer
            width="100%"
            height={Math.max(300, behaviorComparisonData.length * 40)}
          >
            <BarChart
              data={behaviorComparisonData}
              layout="vertical"
              margin={{ left: 20 }}
            >
              <CartesianGrid strokeDasharray="3 3" stroke="#e5e7eb" />
        ...
        <ChartCard
          ...
          subtitle="Multiple metrics across personas"
        >
          <ResponsiveContainer
            width="100%"
            height={Math.max(300, behaviorComparisonData.length * 45)}
          >
            <BarChart
              data={behaviorComparisonData}
              layout="vertical"
        ...
        <ChartCard
          title="Cluster Sizes"
          subtitle={`Minimum required: ${minClusterSize} participants`}
        >
          <ResponsiveContainer
            width="100%"
            height={Math.max(200, sizeData.length * 35)}
          >
      ...
      <Card>
        <h3 className="text-lg font-semibold mb-4">
          Predicted Behavioral Outcomes by Persona
        </h3>
              ...
              {clusters.map((c) => (
                <tr key={c.cluster_id} className="border-t hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 text-right font-mono text-green-600">
                    {(
                      (c.behavioral_outcomes?.report_rate?.mean || 0) * 100
                    ).toFixed(1)}
                    %
                  </td>
                  <td className="px-4 py-3 text-right font-mono text-blue-600">
                    {(
                      (c.behavioral_outcomes?.overall_accuracy?.mean || 0) * 100
                    ).toFixed(1)}
                    %
                  </td>
                  <td className="px-4 py-3 text-right font-mono">
                    {(
                      (c.behavioral_outcomes?.hover_rate?.mean || 0) * 100
                    ).toFixed(1)}
                    %
                  </td>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="p-4 bg-gray-50 rounded-lg">
            ...
            <div className="text-2xl font-bold">
              {clusteringResult.metrics?.calinski_harabasz?.toFixed(1) || "-"}
            </div>
          ...
          <div className="p-4 bg-gray-50 rounded-lg">
            ...
            <div className="text-2xl font-bold">
              {clusteringResult.metrics?.davies_bouldin?.toFixed(3) || "-"}
            </div>
            <div className="text-xs text-gray-400">
          ...
          <div className="p-4 bg-gray-50 rounded-lg">
            ...
            <div className="text-2xl font-bold">
              {clusteringResult.metrics?.anova_significant_outcomes || "-"}
            </div>
            <div className="text-xs text-gray-400">
          ...
          <div className="p-4 bg-gray-50 rounded-lg">
            ...
            <div className="text-2xl font-bold">
              {clusteringResult.metrics?.eta_squared_mean?.toFixed(3) || "-"}
            </div>
