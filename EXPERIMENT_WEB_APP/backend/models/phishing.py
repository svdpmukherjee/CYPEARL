...
class Email(BaseModel):
    ...
    # Job cluster & counterbalancing fields
    job_cluster: Optional[str] = None  # None for generic, cluster name for role-matched
    condition_type: str = "generic"  # "generic" or "role_matched"
    factorial_cell: Optional[int] = None  # 1-16 factorial cell number
...
class EmailSession(BaseModel):
    ...
    perceived_relevance: Optional[int] = None  # 1-7: perceived work relevance per email
...
class Participant(BaseModel):
    ...
    # Job cluster & counterbalancing
    job_cluster: Optional[str] = None  # Assigned cluster from JOB_CLUSTERS
    counterbalance_version: Optional[str] = None  # "A" or "B"
    sub_role: Optional[str] = None  # Functional sub-role from SUB_ROLES

    # Survey data
    pre_survey_data: Optional[Dict[str, Any]] = None
    post_survey_data: Optional[Dict[str, Any]] = None
...
class ResponseExport(BaseModel):
    ...
    job_cluster: Optional[str] = None
    condition_type: Optional[str] = None  # "generic" or "role_matched"
    counterbalance_version: Optional[str] = None  # "A" or "B"
